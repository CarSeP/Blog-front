import type { Post } from "../../interfaces/post.interface";
import style from "./PostCard.module.css";

interface Props {
	post: Post;
}

export function PostComponent({ post }: Props) {
	return (
		<article className={style.postCard}>
			<header className={style.postHeader}>
				<img className={style.postImg} src={post.img}></img>
			</header>
			<div className={style.postContent}>
				<div>
					<div className={style.postCategoryContainer}>
						{post.categories.map((category) => {
							return <span className={style.postCategory}>{category}</span>;
						})}
					</div>
					<h3 className={style.postTitle}>{post.title}</h3>
					<span className={style.postDescription}>{post.description}</span>
				</div>
				<footer>
					<div className={style.postSeparator}></div>
					<div>
						<div className={style.postUserPhoto}></div>
						<div>
							<span>Juan</span>
							<span>Enero 12, 2025</span>
						</div>
					</div>
				</footer>
			</div>
		</article>
	);
}
